...
abstract final class PixelIcons {
  ...
  // Плетёная корзина с ручкой — трапеция сужается книзу, поперечная
  // полоса намекает на плетение, чтобы не путаться с иконкой еды/тарелки.
  ...
  // Кружка с паром — однозначно «напиток/кафе», не пересекается по
  // силуэту с плоской тарелкой (food) или корзиной (groceries).
  ...
  // Тарелка сверху (широкий закрашенный ободок, полая середина) — общий
  // символ «еда», отдельный от кафе/кружки и от продуктовой корзины.
  static const food = [
    '..######..',
    '.########.',
    '##......##',
  ...
  // Машина сбоку: скошенное лобовое стекло смещено к одному краю (не
  // симметрично, чтобы не читалось как лицо), корпус и два отдельных
  // колеса с зазором снизу.
